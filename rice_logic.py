...
# ==========================================
# โหมด 2: สำหรับกล้อง (Real-time Camera)
# ==========================================
def process_rice_realtime(frame, dist_val=0.4):
    # ปรับภาพจากกล้องให้กว้าง 600px เพื่อลดภาระการประมวลผล
    h, w = frame.shape[:2]
    new_w = 600
    new_h = int(h * (new_w / w))
    ...
    contours_red, _ = cv2.findContours(mask_red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    mask_tray = np.zeros(frame.shape[:2], dtype="uint8")
    ...
    roi = cv2.bitwise_and(frame, frame, mask=mask_tray) if contours_red else frame
    ...
    for c in contours_rice:
        area = cv2.contourArea(c)
        # ปรับเกณฑ์ขนาดขั้นต่ำลงมานิดหน่อยสำหรับกล้อง
